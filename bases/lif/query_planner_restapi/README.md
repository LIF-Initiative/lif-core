...
**Partial answers are marked (#1232).** When the planner answers from cache without the fields it was asked for, both query endpoints still return `200` with the records, plus an `X-LIF-Partial` header naming the reason: `no_sources_available` (no configured source serves the fields, so a retry won't help) or `orchestrator_submission_failed` (transient, so a retry may succeed). If the submission failed and the cache held **nothing**, the endpoint returns `503` instead, because an empty `200` reads as "no such learner". Fields still missing after orchestration has run are **not** marked, since that most likely means the learner has no data for them.
