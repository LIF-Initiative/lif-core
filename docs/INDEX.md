...
- [`mdr-import-export-portability.md`](operations/proposals/mdr-import-export-portability.md) — Epic #1223 plan: make MDR schemas and mappings portable between installs — no database IDs in files, edit-by-upload, one converter between file and rows. Grounds three live export failures, the ID-based upload lookups (#762–#765) and five kinds of reference; flags six gaps no ticket covers (inclusions and value mappings never exported, no schema update path, empty mapping groups, hand-synced seed files, unbounded parent chains, multi-target mappings). Five decisions, phased plan, eleven new tickets, four tickets that do not serve portability; rejects a document-store migration.
