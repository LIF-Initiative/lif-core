...
**One name to watch:** the container variable is `LIF_ADVISOR_LLM_MODEL_NAME`, which is what `langchain_agent/core.py:32` reads — but all four compose surfaces populate it from a *differently named* host variable: `LIF_ADVISOR_LLM_MODEL_NAME: ${LANGCHAIN_LLM_MODEL_NAME:-gpt-4.1-mini}` (`development/docker-compose.yml:178`, `development/advisor-demo-1org/docker-compose.yml:132`, `development/advisor-demo-3orgs/docker-compose.yml:250`, `deployments/advisor-demo-docker/docker-compose.yml:422`). So an operator who sets `LIF_ADVISOR_LLM_MODEL_NAME` in a demo `.env` gets no change; the var to set is `LANGCHAIN_LLM_MODEL_NAME`. The ECS task-def (`cloudformation/lif-advisor-api-taskdef-includes.yml:10`) and the dev script (`development/scripts/run_lif_advisor_restapi.sh:11`) set the container name directly and are unaffected. This is the same drift class as the size caps below — note it before adding the R1 sampling vars, which should use one name throughout.
...
1. **R1 (high, finding F4/F5)** — Env-configurable generation params (`LIF_ADVISOR_LLM_TEMPERATURE`, `_TOP_P`, `_PRESENCE_PENALTY`, `_FREQUENCY_PENALTY`) applied at **both** ChatOpenAI sites. Justification is consistency/reproducibility across the two call sites (variance 0.750→0.888), not identifier safety.
   **The default temperature is deliberately left unset by this recommendation.** An earlier draft proposed `0.1`, but Part B is the only end-to-end retrieval measurement here and it does not support that value: against `@1.0`, `@0.1` ties on three of five queries (skills 17/17, courses 2/2, relocate 43/43) and regresses on two — email 1 → 2, and materially, advising session 8 → 49. The consistency argument supports being *able* to set both sites to one value; it does not select the number. Pick the default when the sweep is rebuilt and committed (see R3). The implementation plan therefore defaults to `0.0`, which preserves the agent's current hardcoded value rather than choosing a new one. Plan: [`advisor-llm-sampling-params.md`](../../operations/proposals/advisor-llm-sampling-params.md).
...
3. **R3 (medium, finding F1)** — Keep `SEMANTIC_SEARCH__TOP_K=200` short-term; re-run the sweep after R2 lands, consider 150. **"Consider 150" is a real trade, not a free one:** the recall curve above puts k=150 at 81% against k=200's 92.5% — ~11.5 points. F1's point is that the leaves recovered between 150 and 200 rank low *because* their descriptions are boilerplate, but they are still gold-set members, so dropping k loses them. Two things change the arithmetic, which is why this needs re-measuring rather than deciding from the curve above: the curve was measured *pre*-filter, so it prices in the ~25–30% of slots R2 frees up, and R4 would move the boilerplate leaves' ranks outright. Neither effect's size can be recovered from the surviving figures — the harness is gone (see Operational Notes) — so the honest position is that 150 may well be cheap after R2/R4 and is not cheap today. Note also that R2 landing *requires* k below 186 to keep ranking meaningful (see the plan's Change Set B), so the two are coupled.
